...
def check_qd_qc(act_id):
    # 已经报名并且到了签到时间和签退时间
    if bm_status == '未报名':
        # qd_data_string = str(temp_dir["hdsj"])
        qd_data_string = '2024-03-11 20:59'
        qd_datetime = datetime.strptime(qd_data_string, "%Y-%m-%d %H:%M")
        qc_datetime = qd_datetime + timedelta(hours = qd_qc_offset_hour, minutes = qd_qc_offset_minutes)
        print(datetime.now())
        print(qd_datetime)
        print(qc_datetime)
        if datetime.now() >= qd_datetime:
            # 签到推送
            qd_post_data = {
                'text': '该签到啦！！！',
                'desp': f'{qd_url}{act_id}'
                }
            requests.post(xia_post_url, data = qd_post_data)
        if datetime.now() >= qc_datetime:
            # 签退推送
            qc_post_data = {
                'text': '该签退啦！！！',
                'desp': f'{qc_url}{act_id}'
                }
            requests.post(xia_post_url, data = qc_post_data)
    ...
    try:
        if '登录' in response.text:
            ...
            if len(act_list) > last_act_num:
                ...
                # q酱推送
                # qmsg_cont_url = qmsg_post_url + post_content
                # requests.get(qmsg_cont_url)
