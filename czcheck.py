...
def new_post_content(act_list):
    ...
    return post_content
def get_now_datetime(is_format = False):
    if is_format:
        return datetime.now().strftime(time_format)
    return datetime.now()
start_flag = False
start_hour = get_now_datetime().hour
def get_run_time():
    time_elapsed = get_now_datetime() - program_start_time
...
def send_message(title, message):
    requests.post(xia_post_url, data = {'text': title, 'desp': message})
...
while True:
    ...
    try:
        ...
        server_message = (f'<b>异常内容:</b> {error_msg}\n'
                        f'<b>发生时间:</b> {get_now_datetime(True)}\n'
                        f'<b>执行次数:</b> {loop_num} 次\n')
        send_message(title = '服务器断网', message = server_message)
        # 服务器断网，结束请求
        break
    try:
        if '登录' in response.text:
            ...
            if len(act_list) > last_act_num:
                ...
                send_message(title = '新活动来啦！！！',
                             message = (f'<b>报名状态:</b> {bm_status}\n'
                                        f'{new_act_content}'))
        ...
        if exception_flag:
            # 异常信息推送
            error_message = (f'<b>异常内容:</b> {error_msg}\n'
                            f'<b>发生时间:</b> {get_now_datetime(True)}\n'
                            f'<b>执行次数:</b> {loop_num} 次\n')
            send_message(title = '异常信息', message = error_message)
            # break
        else:

            current_hour = get_now_datetime().hour
            if start_flag is False and current_hour == start_hour:
                # 启动推送消息
                send_message(title = '程序正常启动', message = 'nothing to show!')
                start_flag = True
            # 推送消息周期 当前小时在推送列表中 且 未推送过
            if current_hour in post_message_hour_list and post_message_hour_dir[current_hour] is False:
                # 推送正常消息
                normal_message = (f"<b>报告时间:</b> {get_now_datetime(True)}\n"
                                  f"<b>已运行时间:</b> {get_run_time()}\n"
                                  f"<b>执行时间:</b> {loop_num} 次")
                send_message(title = '正常报告', message = normal_message)
